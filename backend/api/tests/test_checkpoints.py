...
@pytest.mark.anyio
async def test_checkpoints_support_crud(client) -> None:
    ...
    list_response = await client.get("/checkpoints")
    ...
    delete_response = await client.delete(f"/checkpoints/{checkpoint_id}")

    assert delete_response.status_code == 204
    assert (await client.get("/checkpoints")).json() == []
