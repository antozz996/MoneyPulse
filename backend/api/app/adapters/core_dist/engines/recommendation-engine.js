...
function getPurchaseLabel(purchase) {
    return purchase.description ? `"${purchase.description}"` : "This purchase";
}
export function recommend(snapshot, purchase) {
    ...
    if (!purchase) {
        if (risk.projectedRiskLevel === "hold") {
            return {
                ...
                reasons: [
                    `Current safe-to-spend headroom is ${formatMoneyLabel(timeline.availableToSpend.amount, timeline.availableToSpend.currency)}.`,
        ...
        return {
            kind: "daily-safe-to-spend",
            headline: `You can safely spend up to ${formatMoneyLabel(timeline.availableToSpend.amount, timeline.availableToSpend.currency)} today.`,
            action: "Stay within today's protected headroom to preserve the documented buffer and goals.",
            riskLevel: risk.projectedRiskLevel,
            primaryAmount: timeline.availableToSpend,
            reasons: [
                `The deterministic baseline leaves ${formatMoneyLabel(timeline.availableToSpend.amount, timeline.availableToSpend.currency)} for discretionary spending today.`,
    ...
    if (risk.projectedRiskLevel === "safe") {
        return {
            ...
            reasons: [
                `Remaining headroom after the scenario is ${formatMoneyLabel(risk.remainingHeadroom.amount, risk.remainingHeadroom.currency)}.`,
                goalImpact.summary,
    ...
    if (risk.projectedRiskLevel === "caution") {
        return {
            ...
            primaryAmount: risk.remainingHeadroom,
            reasons: [
                `Remaining headroom after the scenario falls to ${formatMoneyLabel(risk.remainingHeadroom.amount, risk.remainingHeadroom.currency)}.`,
                goalImpact.summary,
    ...
    return {
        ...
        reasons: [
            `The scenario overshoots current discretionary headroom by ${formatMoneyLabel(risk.shortfall.amount, risk.shortfall.currency)}.`,
            goalImpact.summary,
