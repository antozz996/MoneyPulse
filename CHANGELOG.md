...
- Fixed the live backend decision-engine bridge for Vercel by bundling a local JavaScript runtime under `backend/api/app/adapters`, preferring that runtime before the workspace TypeScript loader path, and explicitly including the adapter assets in the Vercel Python function package so authenticated finance endpoints like `/today` and `/before-you-buy` no longer depend on `tsx` or monorepo files outside the API project root.
